...
namespace StudentManage.Controllers
{
    public class UnionController : BaseController
    {
        ...
        {
            var list = new UnionBUS().GetUnionBookByCondition(model.start,model.length,classId,unionId,status,facultyId,semester);
            var total = new UnionBUS().getTotalRecord();
            if (list != null)
                return Json(new
                {
